...
class SignUpViewModel @Inject constructor(
    ...
    private fun createUser(email: String) = viewModelScope.launch {
        repository.createUser(email).collect { result ->
            when (result) {
                is Resource.Success -> {
                    _signUpState.send(SignUpState(isSuccess = "User Created"))
                }
                is Resource.Loading -> {
                    _signUpState.send(SignUpState(isLoading = true))
                }
